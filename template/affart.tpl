...
<div class="container">
    <div class="row">
        <div class="col-lg-12 text-center">
            <h1 class="mt-5">{$commen.0.titre}</h1>
        </div>
    </div>
    ...
    <div class="container-fluid">
        <div class="text-center">
            <img src="img/{$commen.0.id}.jpg" alt="" class="img-thumbnail"></div>
    </div>
    </br>

    <div class="row">
        <div class="col-12 text-center">
            <p class="card-text">{$commen.0.texte}</p>
        </div>
    </div>  </br>
    ...
    <div class="row">
        <div class="col-lg-12 text-center">
<!--            Modifier article        -->
            <a href="article.php?id={$commen.0.id}&action=modifier" class="btn btn-primary">Modifier article</a>         
            <form action='affart.php' method="POST"></br>
                <input type="hidden" value="{$commen.0.id}" name="id_article">   
<!--            Supprimer article        -->                                                        <!-- afficher confirmation-->
    ...
    {foreach from=$commen item=value}
        <a><font color="#ff0000">{$value.pseudo}</font> <font size="2">{$value.date}  {$value.temp}</font></a></br>
        <a>{$value.texte_comm}</a>
        <hr>
    {/foreach}
